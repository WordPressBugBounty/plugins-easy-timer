...
if ((easy_timer_data('javascript_enabled') == 'yes') && (strstr($content[$k], 'timer]'))) { add_action('wp_footer', 'easy_timer_lang_js'); add_action('wp_footer', 'easy_timer_js'); }

if ($way == 'up') {
$content[$k] = easy_timer_timer_replace($S[$k], '', 'up', $offset, $content[$k]);
$content[$k] = easy_timer_timer_replace($S[1], 'total-', 'up', $offset, $content[$k]); }
if ($way == 'down') {
$content[$k] = easy_timer_timer_replace(-$S[$i], '', 'down', $offset, $content[$k]);
$content[$k] = easy_timer_timer_replace(-$S[$n - 1], 'total-', 'down', $offset, $content[$k]); }

$content[$k] = easy_timer_timer_replace($S[$k], 'elapsed-', 'up', $offset, $content[$k]);
$content[$k] = easy_timer_timer_replace($S[1], 'total-elapsed-', 'up', $offset, $content[$k]);
$content[$k] = easy_timer_timer_replace(-$S[$i], 'remaining-', 'down', $offset, $content[$k]);
$content[$k] = easy_timer_timer_replace(-$S[$n - 1], 'total-remaining-', 'down', $offset, $content[$k]);
